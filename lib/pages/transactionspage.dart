...
class TransactionHistoryPage extends StatefulWidget {
  ...
  const TransactionHistoryPage({super.key, this.initialIndex = 1});
...
class _TransactionHistoryPageState extends State<TransactionHistoryPage> {
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      ...
      case "Failed":
        return Color(0xFFFFE6E4);
      case "Processing":
        return Color(0xFFFFF6D6);
      default:
        return Colors.grey;
    }
  }
  ...
  Color _getFontColor(String status) {
    switch (status) {
      ...
      case "Failed":
        return Color(0xFFCF0F00);
      case "Processing":
        return Color(0XFFCD7B00);
      default:
        return Colors.grey;
    }
  }
  ...
  void _onItemTapped(int index) {
    ...
    // Handle navigation between tabs (replace with real navigation later)
    if (index == 0) {
      Navigator.pop(context); // go back to Account/Home
    } else if (index == 2) {
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  _buildDetailRow("Date & Time", tx['date']),
                  _buildDetailRow("USD Amount Sent", "\$${tx['usd']}"),
                  _buildDetailRow("INR Amount Deposited", "₹${tx['inr']}"),
                  _buildDetailRow("India Bank Account", tx['account']),
                  _buildDetailRow(
                    "Status",
                    "",
                    valueWidget: Container(
  ...
  Widget _buildDetailRow(String title, String value, {Widget? valueWidget}) {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(title, style: GoogleFonts.poppins(color: Color(0XFF70707B))),
          valueWidget ??
              Text(
                value,
                style: GoogleFonts.poppins(
